...
document.getElementById("topbar-container").innerHTML = renderTopbar("Global Appointments");
...
const renderAppointments = () => {
  ...
  if (!rows.length) {
    tableBodyEl.innerHTML = `
      <tr>
        <td colspan="8" style="text-align:center;color:var(--text-500)">No appointments found for current filters.</td>
      </tr>`;
    return;
  }
...
const loadDoctorsForModal = async () => {
  ...
  allDoctors = (data.users || []).filter((user) => user.role === "doctor" && user.isActive);
...
const loadAppointments = async () => {
  if (tableBodyEl) {
    tableBodyEl.innerHTML = `
      <tr>
        <td colspan="8" style="text-align:center;color:var(--text-500)">Loading appointments...</td>
      </tr>`;
  }
