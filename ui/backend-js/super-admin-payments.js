...
const metaKey = "super_admin_payment_meta";
...
const renderStats = () => {
  ...
  const insuranceAmount = payments
    .filter((payment) => String(paymentMethodLabel(payment)).toLowerCase().includes("insurance"))
    .reduce((sum, payment) => sum + Number(payment.amount || 0), 0);
...
const updateBreakdownCard = () => {
  ...
  const summary = { insurance: 0, creditCard: 0, cash: 0, other: 0 };
  payments.forEach((payment) => {
    const amount = Number(payment.amount || 0);
    const method = String(paymentMethodLabel(payment)).toLowerCase();
    if (method.includes("insurance")) summary.insurance += amount;
    else if (method.includes("credit")) summary.creditCard += amount;
  ...
  const body = breakdownCard.querySelector("div[style*='display:flex;flex-direction:column;gap:12px']");
  if (!body) return;
...
const loadPatients = async () => {
  ...
  patientsById = (data.users || []).reduce((acc, user) => {
...
const loadPaymentConfig = async () => {
  try {
    ...
    paymentConfig = { mockMode: Boolean(data.mockMode) };
  } catch {
