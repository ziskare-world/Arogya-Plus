...
const renderStats = () => {
  ...
  if (subtitleEl) {
    subtitleEl.textContent = `${total.toLocaleString(
      "en-IN"
    )} patients with appointment/consultation in your hospital`;
  }
};
...
const loadData = async () => {
  ...
  const appointmentsLoaded = requests[1].status === "fulfilled";
  ...
  const appointmentPatientIds = new Set(
    appointmentsScopedByDoctor.map((appointment) => extractId(appointment?.patient)).filter(Boolean)
  );

  allAppointments = appointmentsScopedByDoctor;
  if (appointmentsLoaded) {
    allPatients = loadedPatients.filter((patient) => appointmentPatientIds.has(extractId(patient)));
  } else {
